...
if [[ $CHANNEL == "archive" ]]; then
  ...
  docker exec -u www-data prestashop_autoupgrade php modules/autoupgrade/bin/console upgrade:start --admin-dir="admin-dev" --action="compareReleases" --config-file-path="modules/autoupgrade/config.json"
  docker exec -u www-data prestashop_autoupgrade php modules/autoupgrade/bin/console upgrade:start --admin-dir="admin-dev" --config-file-path="modules/autoupgrade/config.json" --chain
fi

docker exec -u www-data prestashop_autoupgrade php modules/autoupgrade/bin/console upgrade:start --admin-dir="admin-dev" --action="compareReleases"
docker exec -u www-data prestashop_autoupgrade php modules/autoupgrade/bin/console upgrade:start --admin-dir="admin-dev" --chain

...
class Download extends AbstractTask
{
    public function run()
    {
        if (!\ConfigurationTest::test_fopen() && !\ConfigurationTest::test_curl()) {
            $this->logger->error($this->translator->trans('You need allow_url_fopen or cURL enabled for automatic download to work. You can also manually upload it in filepath %s.', array($this->upgradeClass->getFilePath()), 'Modules.Autoupgrade.Admin'));
        ...
        $this->logger->debug($this->translator->trans('Downloading from %s', array($this->upgradeClass->upgrader->link), 'Modules.Autoupgrade.Admin'));
        $this->logger->debug($this->translator->trans('File will be saved in %s', array($this->upgradeClass->getFilePath()), 'Modules.Autoupgrade.Admin'));
        ...
        if (\ConfigurationTest::test_dir($relative_download_path, false, $report)) {
            ...
            $this->logger->error($this->translator->trans('Download directory %s is not writable.', array($this->upgradeClass->downloadPath), 'Modules.Autoupgrade.Admin'));
            $this->upgradeClass->next = 'error';
        }
    }
}
...
class Download extends AbstractTask
{
    public function run()
    {
        if (!\ConfigurationTest::test_fopen() && !\ConfigurationTest::test_curl()) {
            $this->logger->error($this->upgradeClass->getTranslator()->trans('You need allow_url_fopen or cURL enabled for automatic download to work. You can also manually upload it in filepath %s.', array($this->upgradeClass->getFilePath()), 'Modules.Autoupgrade.Admin'));
        ...
        if (\ConfigurationTest::test_dir($relative_download_path, false, $report)) {
            ...
            if ($res) {
                ...
                if ($md5file == $this->upgradeClass->upgrader->md5) {
                    ...
                    $this->logger->debug($this->upgradeClass->getTranslator()->trans('Download complete.', array(), 'Modules.Autoupgrade.Admin'));
                    $this->logger->info($this->upgradeClass->getTranslator()->trans('Download complete. Now extracting...', array(), 'Modules.Autoupgrade.Admin'));
                } else {
                    $this->logger->error($this->upgradeClass->getTranslator()->trans('Download complete but MD5 sum does not match (%s).', array($md5file), 'Modules.Autoupgrade.Admin'));
                    $this->logger->info($this->upgradeClass->getTranslator()->trans('Download complete but MD5 sum does not match (%s). Operation aborted.', array(), 'Modules.Autoupgrade.Admin'));
                    $this->upgradeClass->next = 'error';
                }
            } else {
                if ($this->upgradeClass->upgrader->channel == 'private') {
                    $this->logger->error($this->upgradeClass->getTranslator()->trans('Error during download. The private key may be incorrect.', array(), 'Modules.Autoupgrade.Admin'));
                } else {
                    $this->logger->error($this->upgradeClass->getTranslator()->trans('Error during download', array(), 'Modules.Autoupgrade.Admin'));
                }
                $this->upgradeClass->next = 'error';
            }
        } else {
            $this->logger->error($this->upgradeClass->getTranslator()->trans('Download directory %s is not writable.', array($this->upgradeClass->downloadPath), 'Modules.Autoupgrade.Admin'));
            $this->upgradeClass->next = 'error';
        }
    }
}
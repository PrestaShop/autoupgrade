    ...
    public function run()
    {
        ...
        switch ($channel) {
            case 'directory':
                ...
                $this->logger->debug($this->upgradeClass->getTranslator()->trans('Skip downloading and unzipping steps, upgrade process will now remove sample data.', array(), 'Modules.Autoupgrade.Admin'));
                $this->logger->info($this->upgradeClass->getTranslator()->trans('Shop deactivated. Removing sample files...', array(), 'Modules.Autoupgrade.Admin'));
                break;
            case 'archive':
                $this->upgradeClass->next = 'unzip';
                $this->logger->debug($this->upgradeClass->getTranslator()->trans('Skip downloading step, upgrade process will now unzip the local archive.', array(), 'Modules.Autoupgrade.Admin'));
                $this->logger->info($this->upgradeClass->getTranslator()->trans('Shop deactivated. Extracting files...', array(), 'Modules.Autoupgrade.Admin'));
                break;
            default:
                $this->upgradeClass->next = 'download';
                $this->logger->info($this->upgradeClass->getTranslator()->trans('Shop deactivated. Now downloading... (this can take a while)', array(), 'Modules.Autoupgrade.Admin'));
                ...
                $this->logger->debug($this->upgradeClass->getTranslator()->trans('Downloaded archive will come from %s', array($this->upgradeClass->upgrader->link), 'Modules.Autoupgrade.Admin'));
                $this->logger->debug($this->upgradeClass->getTranslator()->trans('MD5 hash will be checked against %s', array($this->upgradeClass->upgrader->md5), 'Modules.Autoupgrade.Admin'));
        }
    }
}
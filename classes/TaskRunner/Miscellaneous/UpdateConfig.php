    ...
    public function run()
    {
        ...
        if (!empty($this->upgradeClass->currentParams['archive_prestashop'])) {
            ...
            if (!file_exists($this->upgradeClass->downloadPath.DIRECTORY_SEPARATOR.$file)) {
                ...
                $this->upgradeClass->next_desc = $this->upgradeClass->getTranslator()->trans('File %s does not exist. Unable to select that channel.', array($file), 'Modules.Autoupgrade.Admin');
                return false;
            }
            if (empty($this->upgradeClass->currentParams['archive_num'])) {
                $this->upgradeClass->error = 1;
                $this->upgradeClass->next_desc = $this->upgradeClass->getTranslator()->trans('Version number is missing. Unable to select that channel.', array(), 'Modules.Autoupgrade.Admin');
                return false;
            }
            ...
            $this->upgradeClass->next_desc = $this->upgradeClass->getTranslator()->trans('Upgrade process will use archive.', array(), 'Modules.Autoupgrade.Admin');
        }
        ...
        if (!$this->upgradeClass->writeConfig($config)) {
            $this->upgradeClass->error = 1;
            $this->upgradeClass->next_desc = $this->upgradeClass->getTranslator()->trans('Error on saving configuration', array(), 'Modules.Autoupgrade.Admin');
        }
    }
}